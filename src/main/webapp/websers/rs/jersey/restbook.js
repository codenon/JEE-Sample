var bookRestUrl = '/JEE-Sample/restfulapi/books';
function rest(restUrl, httpMethod, param, contenttype, datatype, callback) {
	...
	request
			.fail(function(textStatus, errorThrown) {
				jQuery('#resultDiv')
						.html(errorThrown + " status=" + textStatus.status);
			});
}
/* GET */
function getAllBook() {
...
function renderQueryGet(data) {
	$('#resultDiv')
			.html("query result: " + data.bookId + "-" + data.bookName + "-" + data.publisher);
}
function renderPathGet(data) {
	$('#resultDiv')
			.html("path result: " + data.bookId + "-" + data.bookName + "-" + data.publisher);
}
